...
// Enhanced commands - All new features (ADDED handleAntiBotMessage here)
const {
    approveCommand,
    disapproveCommand,
    getGroupPPCommand,
...
async function handleMessages(sock, messageUpdate, printLog) {
    ...
    try {
        ...
        const userMessageCheck = (
            message.message?.conversation || 
            message.message?.extendedTextMessage?.text || 
            ''
        ).trim();
        
        // തിരുത്തിയ ഭാഗം (userMsgCheck എന്നത് userMessageCheck എന്നാക്കി മാറ്റിയിരിക്കുന്നു)
        if (userMessageCheck === '1' || userMessageCheck === '2') {
        ...
        // Handle anti-bot check for messages sent by existing group members (other active bots)
        if (isGroup && !message.key.fromMe) {
            const isDetectedAsBot = await handleAntiBotMessage(sock, chatId, message, senderId, userMessage);
            if (isDetectedAsBot) return;
        }
