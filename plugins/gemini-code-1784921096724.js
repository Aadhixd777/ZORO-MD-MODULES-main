...
// Convert Buffer (MP3) to Opus Voice Note
...
async function handleMentionDetection(sock, chatId, message) {
    try {
        ...
        if (audioBuffer) {
            try {
                // Convert Audio to Opus for Voice Note
                let opusBuffer = audioBuffer;
                try {
                    opusBuffer = await convertToOpus(audioBuffer);
                } catch (e) {
                    console.log('FFmpeg conversion failed, sending raw buffer:', e.message);
                }
                ...
                console.log(`Mention voice note sent successfully`);
                return;
            } catch (e) { 
