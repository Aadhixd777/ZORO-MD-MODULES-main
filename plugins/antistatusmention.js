...
// Anti-Status Mention Plugin Import (All lowercase to match antistatusmention.js)
...
async function handleMessages(sock, messageUpdate, printLog) {
    ...
    try {
        ...
        // Anti-Status Mention Check (Executes for every message in groups)
        if (isGroup) {
            try {
                await handleAntiStatusMention(sock, chatId, message, senderId);
            } catch (err) {
                console.error("Error in handleAntiStatusMention:", err);
            }
        }
        ...
        switch (true) {
            // Anti-Status Mention Toggle Command Added Here
            case userMessage.startsWith('.antistatus'): {
                ...
                const adminStatus = await isAdmin(sock, chatId, senderId, message);
                await toggleAntiStatusMention(sock, chatId, userMessage, senderId, adminStatus.isSenderAdmin);
