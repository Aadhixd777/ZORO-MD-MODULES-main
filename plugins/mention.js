...
function loadState() {
    try {
        ...
        if (state && typeof state.mediaPath === 'string' && state.mediaPath.endsWith('media/mention_default.webp')) {
            return { enabled: !!state.enabled, mediaPath: '', type: 'text' };
        }
        return state;
    } catch {
        // Return default state if file does not exist
        return { enabled: false, mediaPath: '', type: 'text' };
    }
}

// Automatically creates data folder and mention.json file if missing
...
async function ensureDefaultSticker(state) {
    try {
        const mediaPath = path.join(__dirname, '..', state.mediaPath);
        if (state.mediaPath.endsWith('mention_default.webp') && !fs.existsSync(mediaPath)) {
            const defaultStickerPath = path.join(__dirname, '..', 'media', 'stickintro.webp');
            if (fs.existsSync(defaultStickerPath)) {
                fs.copyFileSync(defaultStickerPath, mediaPath);
            } else {
                const mediaDir = path.dirname(mediaPath);
                if (!fs.existsSync(mediaDir)) {
                    fs.mkdirSync(mediaDir, { recursive: true });
                }
                fs.writeFileSync(mediaPath.replace('.webp', '.txt'), 'Default mention sticker not available');
            }
        }
    } catch (e) {
...
async function handleMentionDetection(sock, chatId, message) {
    try {
        ...
        // Custom thumbnail path for external ad reply (media/custom_thumb.jpg)
        ...
        if (!state.mediaPath) {
            await sock.sendMessage(chatId, { text: 'Hi', contextInfo: thumbBuffer ? contextInfo : undefined }, { quoted: message });
            return;
        }
        ...
        try {
            if (state.type === 'sticker') {
                await sock.sendMessage(chatId, { sticker: fs.readFileSync(mediaPath) }, { quoted: message });
                return;
            }

            const payload = {};
            if (state.type === 'image') payload.image = fs.readFileSync(mediaPath);
            else if (state.type === 'video') {
                payload.video = fs.readFileSync(mediaPath);
                if (state.gifPlayback) payload.gifPlayback = true;
            }
            else if (state.type === 'audio') {
                payload.audio = fs.readFileSync(mediaPath);
                payload.mimetype = state.mimetype || 'audio/mpeg';
                if (typeof state.ptt === 'boolean') payload.ptt = state.ptt;
                
                // Attach custom thumbnail layout if audio and thumbnail buffer exist
                if (thumbBuffer) {
                    payload.contextInfo = contextInfo;
                }
            }
            else if (state.type === 'text') payload.text = fs.readFileSync(mediaPath, 'utf8');
...
async function setMentionCommand(sock, chatId, message, isOwner) {
    ...
    try {
        ...
        if (stateBefore.mediaPath && stateBefore.mediaPath.startsWith('media/') &&
            !stateBefore.mediaPath.endsWith('mention_default.webp')) {
            const prevPath = path.join(__dirname, '..', stateBefore.mediaPath);
            if (fs.existsSync(prevPath)) {
                try { fs.unlinkSync(prevPath); } catch {}
            }
        }
    } catch (e) {
        console.warn('cleanup previous media failed:', e?.message || e);
    }
