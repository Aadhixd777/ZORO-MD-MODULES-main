...
async function handleLinkDetection(sock, chatId, message, userMessage, senderId) {
    try {
        ...
        if (linkPattern.test(userMessage)) {
            ...
            // Step 2: കോൺഫിഗറേഷൻ അനുസരിച്ച് ആക്ഷൻ ചെയ്യുക (Kick ആയാൽ മെൻഷനും ടെക്സ്റ്റും പിന്നാലെ ഓഡിയോയും കൊടുക്കുക)
            if (actionType === 'kick') {
                try {
                    // ആദ്യം മെൻഷൻ ചെയ്തുകൊണ്ട് ടെക്സ്റ്റ് മെസ്സേജ് അയക്കുന്നു
                    await sock.sendMessage(chatId, {
                        text: `_@${senderId.split('@')[0]} Uriyil aadunnathinte parayilladathe irangipoda_`,
                        mentions: [senderId]
                    });
                    ...
                    // തൊട്ടുപിന്നാലെ ഓഡിയോ ഫയൽ അയക്കുന്നു
                    await sock.sendMessage(chatId, {
                        audio: { url: audioUrl },
                        mimetype: 'audio/mpeg',
                        ptt: false
                    ...
                    console.log('Text, mention and audio sent successfully. Waiting before kicking...');
                    
                    // യൂസർക്ക് മെസ്സേജും ഓഡിയോയും പൂർണ്ണമായി കാണാനും കേൾക്കാനും ഡിലേ (3 സെക്കൻഡ്)
                    await new Promise(resolve => setTimeout(resolve, 3000));

                    // Step 3: അതിനുശേഷം മാത്രം യൂസറെ ഗ്രൂപ്പിൽ നിന്ന് കിക്ക് ചെയ്യുന്നു
                    await sock.groupParticipantsUpdate(chatId, [senderId], 'remove');
                    console.log('User kicked successfully after text and audio.');
                } catch (err) {
